...
cal27_data_ann = bind_cols(rep(1:16,each=24),rep(1:24,16),cal27_data)
cal27_data_ann = bind_cols(rep(rep(1:16,each=24),5), rep(rep(1:24,16),5) ,cal27_data)

colnames(cal27_data_ann) = c("row","col", colnames(cal27_data))
cal27_data = cal27_data %>%  left_join(fadu_drug_map)   %>%  mutate(drugs = case_when(drugs == "DMSO + dmso" ~ "bckg",
                                                                                        drugs == "STAUR + dmso" ~ "staur",
                                                                                        drugs == "dmso wellmate only/unpinned" ~ "dmso",
                                                                                        TRUE ~ drugs))
cal27_data_ann = cal27_data_ann %>%  left_join(fadu_drug_map) %>%  
...
data_controls = read_csv("~/Work/Flavi CBD screen/CBD_synergy_2022/plate_maps/fadu_data_controls.csv",
                           na = "NA")

# now creating 20 sets  of 4  randomly selected dmso wells 
# (with replacement, from the block of 20 wells in cols 3 and 4)
dmso_contr_groups = cal27_data_ann %>% filter(groups == "dmso", col==3 | col == 4, row < 11) %>% 
...
dmso_wells_around_drugs = read_csv("~/Work/Flavi CBD screen/CBD_synergy_2022/plate_maps/dmso_wells_around_drugs.csv")
