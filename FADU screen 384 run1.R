...
plate_controls = read_excel("plate384_controls.xlsx")
...
fadu_data  = fadu_data  %>%  mutate(kind = "character")
fadu_data  = fadu_data  %>%  add_column(kind = NA)




fadu_data2  = fadu_data  %>%  mutate(kind = case_when(fadu_data$groups == "DMSO + dmso" ~ "bckg",
                                                      fadu_data$groups == "STAUR + dmso" ~ "staur",
                                                      fadu_data$groups == "dmso wellmate only/unpinned" ~ "dmso",
                                                      fadu_data$groups == "library 0.2µM + dmso wellmate" ~ "lib",
                                                      fadu_data$groups == "library + dmso" ~ "lib"),
                                     kind_code = case_when(fadu_data$groups == "DMSO + dmso" ~ 0,
                                                      fadu_data$groups == "STAUR + dmso" ~ 10,
                                                      fadu_data$groups == "dmso wellmate only/unpinned" ~ 1,
                                                      fadu_data$groups == "library 0.2µM + dmso wellmate" ~ 2,
                                                      fadu_data$groups == "library + dmso" ~ 2))
