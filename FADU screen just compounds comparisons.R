...
fadu_data_norm_by_cbd_run1 = read_csv("fadu_data_norm_by_cbd run1.csv") %>% arrange(Well)
matrix(filter(fadu_data_norm_by_cbd_run1,comp == 2)$Avg_control,8,12, byrow = T) %>%  
  pheatmap( cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 15,
            breaks = seq(0.5,1.5, length.out = 100),  
            filename = "FADU run1 comp 2000 normalized avg.pdf")
matrix(filter(fadu_data_norm_by_cbd_run1,comp == 200)$Avg_control,8,12, byrow = T) %>%  
  pheatmap( cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 15,
            breaks = seq(0.5,1.5, length.out = 100),  
            filename = "FADU run1 comp 200 normalized avg.pdf")

fadu_data_normalized_controls_run1 = fadu_data_norm_by_cbd_run1 %>% select(Well,well_type, comp, Avg_control,SD_control) %>% 
  pivot_wider(names_from = comp, values_from = c(Avg_control, SD_control))

bind_cols(fadu_data_normalized_controls_run3, select(fadu_data_normalized_controls_run1, Avg_control_2, SD_control_2))  %>% 
  ggplot(aes(x = Avg_control_2, y = Avg_control_2000, color = well_type))+
  geom_point() + geom_smooth(method = 'lm') +
  geom_errorbar(aes(ymin = Avg_control_2000 - SD_control_2000, ymax = Avg_control_2000 + SD_control_2000), width = 0.1) +
  geom_errorbarh(aes(xmin = Avg_control_2 - SD_control_2, xmax = Avg_control_2 + SD_control_2), height = 0.1) +
  coord_cartesian(xlim = c(0,1.5),ylim = c(0,1.5))
ggsave("run3  vs run 1 compounds 2000.png") 

fadu_data_normalized_controls_run1  %>% 
  ggplot(aes(x = Avg_control_2, y = Avg_control_200, color = well_type))+
  geom_point() + geom_smooth(method = 'lm') +
  geom_errorbar(aes(ymin = Avg_control_200 - SD_control_200, ymax = Avg_control_200 + SD_control_200), width = 0.1) +
  geom_errorbarh(aes(xmin = Avg_control_2 - SD_control_2, xmax = Avg_control_2 + SD_control_2), height = 0.1) +
  coord_cartesian(xlim = c(0,1.5),ylim = c(0,1.5))
ggsave("run 1 compounds200 vs 2000.png") 


run1_controls = select(fadu_data_normalized_controls_run1, Avg_control_2, SD_control_2, Avg_control_200, SD_control_200)
colnames(run1_controls) = c("run1_Avg_2000","run1_SD_2000","run1_Avg_200","run1_SD_200")
run3_controls = select(fadu_data_normalized_controls_run3, Well, Avg_control_200, SD_control_200, Avg_control_2000, SD_control_2000, Avg_control_5000, SD_control_5000)
colnames(run3_controls) = c("Well", "run3_Avg_200","run3_SD_200","run3_Avg_2000","run3_SD_2000","run3_Avg_5000","run3_SD_5000")

runs_1_3_controls = bind_cols(run1_controls, run3_controls) %>% select(Well,everything())
runs_1_3_controls %>%    write_csv("compounds alone runs 1 3.csv")
runs_1_3_controls_simplif = runs_1_3_controls  %>%  
  mutate(run1_200_sign = sign(abs(run1_Avg_200-1)-2*run1_SD_200)) %>% 
  mutate(run1_Avg_200 = ifelse(run1_200_sign >0,run1_Avg_200,NA )) %>% 
  mutate(run1_2000_sign = sign(abs(run1_Avg_2000-1)-2*run1_SD_2000)) %>% 
  mutate(run1_Avg_2000 = ifelse(run1_2000_sign >0,run1_Avg_2000,NA )) %>% 
  mutate(run3_200_sign = sign(abs(run3_Avg_200-1)-2*run3_SD_200)) %>% 
  mutate(run3_Avg_200 = ifelse(run3_200_sign >0,run3_Avg_200,NA )) %>% 
  mutate(run3_2000_sign = sign(abs(run3_Avg_2000-1)-2*run3_SD_2000)) %>% 
  mutate(run3_Avg_2000 = ifelse(run3_2000_sign >0,run3_Avg_2000,NA )) %>% 
  mutate(run3_5000_sign = sign(abs(run3_Avg_5000-1)-2*run3_SD_5000)) %>% 
  mutate(run3_Avg_5000 = ifelse(run3_5000_sign >0,run3_Avg_5000,NA )) %>% 
    select(Well, contains("Avg"))
runs_1_3_controls_simplif %>% write_csv("runs_1_3_controls_simplif.csv")
